...
mkdir -p \${TMPDIR}/${id};
cd \${TMPDIR}/${id};
time kb count --h5ad -i ${kref}/index.idx -o ${id} -g ${kref}/t2g.txt -x 10XV2 -c1 ${kref}/spliced_t2c.txt -c2 ${kref}/unspliced_t2c.txt --lamanno --filter bustools --verbose -t 6 ${fastqlist};
mv ${id} ${PWD}/bus/;" | qsub -q erprq -l nodes=1:ppn=8 -l mem=60g -N $sample
    sleep 0.5;
done
