...
sc[["percent.mt"]] <- PercentageFeatureSet(sc, features = anno %>% filter(chr=="MT") %>% select(rn) %>% unlist() )
...
### kb, unfiltered data and removing diem ## default data, 301,637 barcodes
### kb2, unfilered data and removing dime, scs>0 and ucs>0, 304,360 barcodes
### kb3, filtered data ## filtered by bustools, 306,299 barcodes 
...
geneSel <- anno%>%filter(uns)%>%select(rn)%>%unlist()           
Xs <- count[geneSel,]
...
dd <- dd%>%rename(ident=orig.ident)%>%
           mutate(batch=gsub("-.*","",ident))

ii <- colSums(count)
###(1)           
fig1 <- ggplot(dd, aes(x=ident, y=ncell, fill=factor(batch)))+
...
### (2), spliced and unspliced            
dd1 <- dd%>%select(ident,ncell,batch)%>%mutate(stats=1)%>%rename(y=ncell)
dd2 <- dd%>%select(ident,reads,batch)%>%mutate(stats=2)%>%rename(y=reads)
dd3 <- dd%>%select(ident,ngene,batch)%>%mutate(stats=3)%>%rename(y=ngene)
ddnew <- rbind(dd1,dd2,dd3)      

stats <- as_labeller(c("1"="#Barcodes","2"="#Reads per cell","3"="#Genes per cell"))
fig0 <- ggplot(ddnew, aes(x=ident, y=y, fill=factor(batch)))+
        geom_bar(stat="identity")+
        xlab("")+
        scale_y_continuous("", expand=expansion(mult=c(0,0.1)))+
        facet_wrap(~factor(stats), nrow=3, scales="free_y", labeller=stats)+
        geom_text(aes(label=round(y)),vjust=-0.5, size=3)+
        theme_bw()+
        theme(legend.title=element_blank(),
              axis.text.x=element_text(angle=90, hjust=1, size=8),
...
dd1 <- dd%>%select(ident, ncell, batch)%>%mutate(stats=1)%>%rename(y=ncell)
dd2 <- dd%>%select(ident,S_reads, batch)%>%mutate(stats=2)%>%rename(y=S_reads)
dd3 <- dd%>%select(ident,S_ngene, batch)%>%mutate(stats=3)%>%rename(y=S_ngene)
ddnew2 <- rbind(dd1,dd2,dd3)      
